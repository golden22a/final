...
function insert_exp($wd,$cd,$rd,$wa,$ca,$ra ,$dated,$timed,$datea,$timea,$type,$tonage,$prix,$id){
   global $bdd;
    ...
    $dated=$dated.' '.$timed;
    $datea=$datea.' '.$timea;
    $req=$bdd->prepare("INSERT INTO `annonce_prestataire`(`date`,`datea`, `wilaya_d`, `commune_d`, `rue_d`, `id_user`, `tonage`, `wilaya_a`, `commune_a`, `rue_a`, `prix`, `type_camion`) VALUES (:date,:datea,:wd,:cd,:rd,:id,:tonage,:wa,:ca,:ra,:prix,:type)");
    $req->bindParam(':wd',$wd,PDO::PARAM_STR);
    $req->bindParam(':wa',$wa,PDO::PARAM_STR);
     $req->bindParam(':cd',$cd,PDO::PARAM_STR);
    $req->bindParam(':ca',$ca,PDO::PARAM_STR);
    $req->bindParam(':ra',$ra,PDO::PARAM_STR);
    $req->bindParam(':rd',$rd,PDO::PARAM_STR);
    $req->bindParam(':date',$dated,PDO::PARAM_STR);
